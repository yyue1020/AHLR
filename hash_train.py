...
import numpy as np
from utils.calc_utils import contrastive_loss
...
from utils.calc_utils import p_topK
...
class Trainer(TrainBase):
    ...
    def our_loss(self, image, text, label, epoch, times):
        ...
        contrastive_loss_imgtext = contrastive_loss(image, text, label_sim)
        
               
        total_loss = similarity_loss + contrastive_loss_imgtext
       
        
        if self.args.hash_layer != "select":
            quantization_loss = (self.hash_loss(image) + self.hash_loss(text)) / 2
            loss = similarity_loss + quantization_loss + contrastive_loss_imgtext
            if self.global_step % self.args.display_step == 0:
                self.logger.info(f">>>>>> Display >>>>>> [{epoch}/{self.args.epochs}], [{times}/{len(self.train_loader)}]: all loss: {loss.data}, "\
                    ...
                    f"contrastive loss: {contrastive_loss_imgtext.data},"\
                    f"lr: {'-'.join([str('%.9f'%itm) for itm in sorted(list(set(self.optimizer.get_lr())))])}")
        else:
            loss = similarity_loss + contrastive_loss_imgtext# + self.args.qua_gamma * (image_quantization_loss + text_quantization_loss)
            if self.global_step % self.args.display_step == 0:
                self.logger.info(f">>>>>> Display >>>>>> [{epoch}/{self.args.epochs}], [{times}/{len(self.train_loader)}]: all loss: {loss.data}, "\
                    f"SIMILARITY LOSS, Intra, positive: {intra_positive_loss.data}, negitave: {intra_negative_loss.data}, sum: {intra_similarity_loss.data}, " \
                    f"Inter, image positive: {inter_positive_loss_i.data}, image negitave: {inter_negative_loss_i.data}, "\
    ...
    def test(self, mode_name="i2t"):
        if self.args.pretrained == "result/mir/64/train.log":
        ...
        K = np.array([50,100, 200, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000])
        pk_i2t = p_topK(query_img, retrieval_txt, self.query_labels, self.retrieval_labels, K)
        pk_t2i = p_topK(query_txt, retrieval_img, self.query_labels, self.retrieval_labels, K)
        self.max_mapt2i = max(self.max_mapt2i, mAPt2i)
        self.logger.info(f">>>>>> MAP(i->t): {mAPi2t}, MAP(t->i): {mAPt2i}, MAP(t->t): {mAPt2t}, MAP(i->i): {mAPi2i}")

        print('...test p_topk: pk(i->t):', ', '.join(['%.4f' % item for item in pk_i2t]))
        print('...test p_topk: pk(t->i):', ', '.join(['%.4f' % item for item in pk_t2i]))
